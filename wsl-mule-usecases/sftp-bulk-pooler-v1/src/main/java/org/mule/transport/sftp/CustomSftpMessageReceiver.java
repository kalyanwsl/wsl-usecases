...
public class CustomSftpMessageReceiver extends AbstractPollingMessageReceiver
	...
	public void poll() throws Exception {
		if (logger.isDebugEnabled()) {
			logger.debug("Polling. Called at endpoint "
					+ endpoint.getEndpointURI());
		}
		try {
			String[] files = sftpRRUtil.getAvailableFiles(false);
			logger.info("No.of files existing for the current poll: "
					+ files.length);
			if (files.length == 0) {
				if (logger.isDebugEnabled()) {
					logger.debug("Polling. No matching files found at endpoint "
							+ endpoint.getEndpointURI());
				}
			} else {
				if (logger.isDebugEnabled()) {
					logger.debug("Polling. " + files.length
							+ " files found at " + endpoint.getEndpointURI()
							+ ":" + Arrays.toString(files));
				}
				routeFile(files);
				if (logger.isDebugEnabled()) {
					logger.debug("Polling. Routed all " + files.length
							+ " files found at " + endpoint.getEndpointURI());
				}
			}
	...
	protected void doInitialise() throws InitialisationException {
		...
		this.poolOnPrimaryInstanceOnly = Boolean.valueOf(System.getProperty(
				"mule.transport.sftp.singlepollinstance", "false"))
	...
	protected void routeFile(final String[] files) throws Exception {
		...
		executionTemplate.execute(new ExecutionCallback<MuleEvent>() {
			...
			public MuleEvent process() throws Exception {
				...
				// A bit tricky initialization of the notifier in this case
				// since we don't
				// have access to the message yet...
				SftpNotifier notifier = new SftpNotifier(
						(SftpConnector) connector, createNullMuleMessage(),
				...
				for (String file : files) {
					...
					Lock fileLock = lockFactory.createLock(connector.getName()
							+ file);
					if (fileLock.tryLock(10, TimeUnit.MILLISECONDS)) {
						...
						try {
							...
							InputStream inputStream = sftpRRUtil.retrieveFile(
									file, notifier);
							// converts file-inputStream to ByteArray then
							// compress contents and store in collection
							fileData = new HashMap<String, Object>();

							fileData.put("fileName", file);
							fileData.put("fileData",
									compressFileContentstoBA(IOUtils
											.toByteArray(inputStream)));

							cmprsdFilesCollection.add(fileData);
				...
				// message.setOutboundProperty(SftpConnector.PROPERTY_FILENAME,
				// path);
				// message.setOutboundProperty(SftpConnector.PROPERTY_ORIGINAL_FILENAME,
				// path);

				// Now we have access to the message, update the notifier with
				// the message
	...
	protected MuleMessage handleUnacceptedFilter(MuleMessage message) {
		...
		try {
			final SftpInputStream payload = (SftpInputStream) message
					.getPayload();
			payload.close();
		} catch (Exception e) {
